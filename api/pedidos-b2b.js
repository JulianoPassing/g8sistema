  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const [result] = await withTimeout(
        connection.execute(`INSERT INTO pedidos (empresa, descricao, dados, data_pedido) VALUES (?, ?, ?, NOW())`, [
          empresa,
          descricao,
          JSON.stringify(dadosCompletos)
        ]),
        30000,
        'Inserção de pedido B2B'
      );
    ...
    if (req.method === 'GET') {
      ...
      const [rows] = await withTimeout(
        connection.execute(
          `SELECT * FROM pedidos WHERE ${whereCliente} ORDER BY data_pedido DESC LIMIT 500`,
          [cid, cid]
        ),
        30000,
        'Listagem de pedidos B2B'
      );
    ...
    if (connection) {
      try {
        ...
        console.error('Erro ao liberar conexão MySQL (B2B):', closeErr);
      }
    }
  }
};
